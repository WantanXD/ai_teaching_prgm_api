...
router.post("/generateQuestion", async(req, res) => {
    ...
    // 問題を生成
    const generateQuestionPrompt = `プログラミング言語"${pl}"のquestionを1つ出力して。questionとは、プログラミング言語${pl}の知識を試すものであるとする。生成した問題に対する模範解答も生成して。ただし、学生に解答が知られてしまうといけないので、必ず問題と模範解答を"---"で区切ること。`;

    const generateQuestion = await model.generateContentStream(generateQuestionPrompt);

    const responce = await (await generateQuestion.response).text().split("---");
...
router.post("/answerCheck", async(req, res) => {
    ...
    const answerCheckPrompt = `プログラミング言語${pl}に関する問題文「${question}」に対する模範解答「${modelAnswer}」を表す内容として、ユーザの解答「${answer}」が正しい場合は"Y"を出力して。そうでない場合は"Great"を表示して。`;
